...
export default function Home(){
  ...
  return <main className="shell">
    ...
    <Card n="3" title="結合の設定" sub="1つの出力に何枚つなげますか？">
      ...
      <fieldset className="formats"><legend>出力形式</legend><label className={format==="png"?"on":""}><input type="radio" name="f" checked={format==="png"} onChange={()=>setFormat("png")}/><b>PNG</b><span><strong>PNG画像</strong><small>1枚ずつ保存／まとめてZIP</small></span></label><label className={format==="pdf"?"on":""}><input type="radio" name="f" checked={format==="pdf"} onChange={()=>setFormat("pdf")}/><b className="red">PDF</b><span><strong>PDF</strong><small>すべてを1つに保存</small></span></label></fieldset>
