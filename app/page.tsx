...
export default function Home(){
  ...
  return <main className="shell">
    ...
    <Card n="2" title="並び順を確認" sub="左から右へ、この順番で結合します" badge={`${items.length}枚`}>
      {items.length?<div className="thumbs">{items.map((x,i)=><article key={x.id} data-id={x.id} draggable={!busy} onDragStart={()=>drag.current=x.id} onDragOver={e=>e.preventDefault()} onDrop={()=>drag.current&&move(drag.current,x.id)}><i>{i+1}枚目</i><img src={x.thumb} alt={`${i+1}枚目のプレビュー`}/><div><strong title={x.name}>{x.name}</strong><span>{x.kind==="pdf"?`${x.page}ページ目 / PDF`:ext(x.name).toUpperCase()}</span></div><button className="handle" aria-label="ドラッグして並び替え" onPointerDown={e=>{if(e.pointerType!=="mouse"){touch.current=x.id;e.currentTarget.setPointerCapture(e.pointerId)}}} onPointerMove={e=>{if(!touch.current||e.pointerType==="mouse")return;const node=document.elementFromPoint(e.clientX,e.clientY)?.closest("[data-id]") as HTMLElement|null;const over=node?.dataset.id;if(over)move(touch.current,over)}} onPointerUp={()=>touch.current=null}>⠿</button><nav><button disabled={i===0} onClick={()=>move(x.id,items[i-1].id)}>←</button><button disabled={i===items.length-1} onClick={()=>move(x.id,items[i+1].id)}>→</button></nav><button className="remove" onClick={()=>remove(x)}>×</button></article>)}</div>:<Empty text="追加した楽譜がここに並びます"/>}
    </Card>
